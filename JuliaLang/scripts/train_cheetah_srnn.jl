# train_cheetah_srnn.jl — Half-Cheetah Autoregressive Dynamics with SRNNCell (batched BPTT via Zygote)
#
...
#
# Usage:
#   julia --project=JuliaLang JuliaLang/scripts/train_cheetah_srnn.jl [--epochs 200] [--size 32] [--lr 0.01] [--bs 16]
#
...
function build_model(args, rng)
    cell = SRNNCell(args.model_size, OBS_SIZE, args.n_E;
    ...
    # Vector regression head: Dense(n → 17), no activation
    head = Lux.Dense(args.model_size => OBS_SIZE;
...
function train!(cell, head, ps_cell, ps_head, st_cell, st_head, data::CheetahData;
    ...
    for epoch in start_epoch:(epochs - 1)
        ...
        if valid_mse < best_valid_mse && epoch > start_epoch
            ...
            best_path = joinpath(save_dir, "srnn_cheetah_best.jld2")
        ...
        if save_every > 0 && epoch > start_epoch && epoch % save_every == 0
            periodic_path = joinpath(save_dir, @sprintf("srnn_cheetah_epoch_%03d.jld2", epoch))
...
function main()
    args = parse_args()
    println("Cheetah Training — SRNNCell ($(args.solver), batched BPTT)")
