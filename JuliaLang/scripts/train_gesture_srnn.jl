# train_gesture_srnn.jl — Gesture Phase Recognition with SRNNCell (batched BPTT via Zygote)
#
# Adapted from: gesture.py (Hasani et al.)
# Uses SRNNCell with fused semi-implicit or explicit Euler solver.
#
# Usage:
#   julia --project=JuliaLang JuliaLang/scripts/train_gesture_srnn.jl [--epochs 200] [--size 32] [--lr 0.01] [--bs 256]
#
...
function train!(cell, head, ps_cell, ps_head, st_cell, st_head, data::GestureData;
    ...
    for epoch in start_epoch:(epochs - 1)
        ...
        if valid_acc > best_valid_acc && epoch > start_epoch
            ...
            best_path = joinpath(save_dir, "srnn_gesture_best.jld2")
        ...
        if save_every > 0 && epoch > start_epoch && epoch % save_every == 0
            periodic_path = joinpath(save_dir, @sprintf("srnn_gesture_epoch_%03d.jld2", epoch))
...
function main()
    args = parse_args()
    println("Gesture Training — SRNNCell ($(args.solver), batched BPTT)")
