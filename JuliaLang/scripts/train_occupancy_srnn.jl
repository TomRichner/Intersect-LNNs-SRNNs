# train_occupancy_srnn.jl — Occupancy Detection with SRNNCell (batched BPTT via Zygote)
#
# Adapted from: train_har_srnn.jl
# Uses SRNNCell with fused semi-implicit or explicit Euler solver.
#
# Usage:
#   julia --project=JuliaLang JuliaLang/scripts/train_occupancy_srnn.jl [--epochs 200] [--size 32] [--lr 0.01] [--bs 256]
#
...
function train!(cell, head, ps_cell, ps_head, st_cell, st_head, data::OccupancyData;
    ...
    for epoch in start_epoch:(epochs - 1)
        ...
        if valid_acc > best_valid_acc && epoch > start_epoch
            ...
            best_path = joinpath(save_dir, "srnn_occupancy_best.jld2")
        ...
        if save_every > 0 && epoch > start_epoch && epoch % save_every == 0
            periodic_path = joinpath(save_dir, @sprintf("srnn_occupancy_epoch_%03d.jld2", epoch))
...
function main()
    args = parse_args()
    println("Occupancy Training — SRNNCell ($(args.solver), batched BPTT)")
