# train_person_srnn.jl — PersonActivity Classification with SRNNCell (batched BPTT via Zygote)
#
...
#
# Usage:
#   julia --project=JuliaLang JuliaLang/scripts/train_person_srnn.jl [--epochs 200] [--size 32] [--lr 0.001] [--bs 64]
#
...
function train!(cell, head, ps_cell, ps_head, st_cell, st_head, data::PersonData;
    ...
    for epoch in start_epoch:(epochs - 1)
        ...
        if valid_acc > best_valid_acc && epoch > start_epoch
            ...
            best_path = joinpath(save_dir, "srnn_person_best.jld2")
        ...
        if save_every > 0 && epoch > start_epoch && epoch % save_every == 0
            periodic_path = joinpath(save_dir, @sprintf("srnn_person_epoch_%03d.jld2", epoch))
...
function main()
    args = parse_args()
    println("Person Training — SRNNCell ($(args.solver), batched BPTT)")
